class App {
  ...
  async main() {
    ...
    const photographer = mediaData.photographers.filter(
      (photographer) => photographer.id === leId
    );
    const medias = mediaData.media.filter(
      (medias) => medias.photographerId === leId
    );
    ...
    // image modal
    medias.findIndex((e) => e.id === medias[0].id);

    // tableau de likes
    const sumLikes = Array.from(medias, (e) => parseFloat(e.likes));

    const likePriceModel = new LikesPrice(
      allLikes(sumLikes),
    ...
    medias.forEach((media) => {
      ...
      if (value === "0") {
        if (media.video) {
          mediaModel = new multimedia(
            media,
            photographer[0],
            sliceName,
            "video",
            tabIndex(tableauMedia)
          );
        } else {
          mediaModel = new multimedia(
            media,
      ...
      triage2.addEventListener("change", () => {
        ...
        result.forEach((media) => {
          ...
          tabIndex(tabmedia);
          this.likesMedia = new IncrementationLikes(
            photographerModel,
            tabmedia,
            sumLikes
          );
